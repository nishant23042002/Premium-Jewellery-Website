...
/** Records a search hit for the "Popular Searches" feature — called from the /search page render, so it only counts committed searches (not every keystroke of a live-typing dropdown). Silently no-ops on very short queries and swallows errors, since this is a nice-to-have analytics signal, not something that should ever break the search page. */
